...
1) Применим правило записи в $\phi_A$, чтобы удалить везде запись в исходной формуле\newline
2) Заменить везде квантор существования вида $\exists i \in T_I. P(i)$ на $P(j)$, где $j$ - новая переменная\newline
3) Заменить везде квантор всеобщности вида $\forall i \in T_I. P(i)$ на $\bigwedge_{i \in \iota(\phi)}P(i)$\newline
4) Заменить везде чтение из массива на неинтерпритируемые функции
\end{frame}
